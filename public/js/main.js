        ...
        .then((response) => {
            ...
            if (!maintenance) {
                ...
                if (response.config_requested) {
                    ...
                    themeInput.setAttribute('type', 'hidden');
                    themeInput.setAttribute('id', 'widget_theme');
                    themeInput.setAttribute('value', response.widget_theme);
                    widgetRoot.appendChild(themeInput);
                ...
                if (response.entity_count) {
                    let entityCountInput = document.createElement('input');
                    entityCountInput.setAttribute('type', 'hidden');
                    entityCountInput.setAttribute('id', 'pos_entity_count');
                    entityCountInput.setAttribute('value', Math.min(+response.entity_count || 0, 99));
                    widgetRoot.appendChild(entityCountInput);
                }
            ...
            if (response.fonts) {
                response.fonts.forEach((font) => {
                    let fontFaceStyle = document.createElement('style');
                    fontFaceStyle.appendChild(document.createTextNode(font));
